...
int main() {
  ...
  std::string url = std::string(BASE) + "/api/v1/guard/results?project_id=demo&per_page=25";
...
$BASE = "http://sagexai.test";
$TOKEN = "<YOUR_TOKEN>";
$url = "$BASE/api/v1/guard/results?project_id=demo&per_page=25";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let url = format!("{}/api/v1/guard/results?project_id=demo&per_page=25", base);
    let res = Client::new()
        .get(url)
        .bearer_auth(token)
        .send()?;
...
<Tab>
```bash
curl -s -X GET "$BASE/api/v1/guard/results?project_id=demo&per_page=25"   -H "Authorization: Bearer $TOKEN"
```
</Tab>
